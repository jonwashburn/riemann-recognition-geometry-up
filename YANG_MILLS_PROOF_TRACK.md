...
- [x] **Uniform control of \(\mathbb E S_R\) (RG/LEAP)**: made explicit as a named RG-grade assumption in `Yang-Mills.tex` (`assump:uei-mean`, also labeled `lem:U1-C-mean`), so the centering-removal step is no longer a hidden divergence.
...
| 18 | `thm:uei-fixed-region` / `thm:U1-lsi-uei` | RG-grade U1 package (open; now explicitly conditional) | weak-coupling / continuum control (RG) | **Normalization confirmed**: in TeX, \(S_R:=\sum_{p\subset R}\phi(U_p)\) (no extra \(a^4\); weight \(e^{-\beta S_R}\)). The weak-coupling schedule dependence is now explicit, and the “centering removal” step is no longer hidden: `thm:uei-fixed-region` is explicitly conditional on `assump:uei-mean`. Remaining open pieces include: a dimension/stencil-uniform LSI on the tree-gauge chord Gibbs law and the needed small-field/Lipschitz gradient controls (the current appendix proof sketch still uses nontrivial patching and small-field heuristics). See `U1_OPERATIONAL_PLAN.md` for a concrete closure-target decomposition. Treat U1 as a named RG-grade closure target until those ingredients are proved or precisely sourced. |
