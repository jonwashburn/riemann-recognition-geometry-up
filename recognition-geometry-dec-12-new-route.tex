...
\begin{itemize}
    ...
    \item \textbf{Alternate major rebuild (explicit formula / Weil--Li positivity).} If the project pivots away from the boundary-certificate/Carleson toolbox entirely, a clean replacement ``gate'' is to recast RH as an \emph{explicit-formula positivity} problem: Weil's criterion (RH \(\Leftrightarrow\) positivity of the explicit-formula quadratic form \(W^{(1)}(f*\widetilde{\overline f})\) for all test functions \(f\)) or, equivalently, Li's criterion (RH \(\Leftrightarrow\) \(\lambda_n\ge 0\) for all Li coefficients). This route is global rather than local and avoids the \(\mu\)-Carleson packing bottleneck; see \texttt{renormalized\_tail\_bound.md} (\S8).  A cautionary note is that stronger de Branges shift-positivity conditions fail for \(\zeta\) (Conrey--Li, arXiv:math/9812166), so the positivity target must be of Weil/Li ``averaged'' type rather than pointwise.
\end{itemize}
