...
| **Vatsal** (Duke Math. J. **98** (1999), *Canonical periods and congruence formulae*) | Defines canonical periods (well-defined up to \(p\)-adic units under cohomological freeness/Gorenstein hypotheses) and proves congruences of algebraic \(L\)-values and \(p\)-adic \(L\)-functions under mod-\(p^r\) congruences of cuspforms; also treats Eisenstein congruence cases using Washington-type nondivisibility inputs. | Author-hosted PDF: `https://www.math.ubc.ca/~vatsal/research/bah.PDF` (local copy: `tmp/ubc/vatsal1999_canonical_periods.pdf`). Key pointers in that text: Theorem **(0.2)** (canonical periods and congruence of algebraic special values under \(f\equiv g\)); Proposition **(1.7)** (special-value congruences); Theorem **(1.10)** (congruence of \(p\)-adic \(L\)-functions via Weierstrass); Theorem **(2.10)** (Eisenstein congruence case with a \(p\)-adic unit period \(\Omega_E\); uses Washington to show nondivisibility on the Eisenstein side). | Requires the “Condition 1/2” cohomology freeness/Gorenstein-style hypotheses in the irreducible case; Eisenstein case uses residue/integrality conditions and Washington nondivisibility inputs. | **Does not (by itself) close Fpμ‑2 for general non‑CM \(E\)**: it supplies canonical-period + congruence machinery and some **special** base cases (Eisenstein-style congruences), but it does not state the needed cyclotomic primitivity bridge “\(p\mid L_p(E,T)\Rightarrow p\mid C(E)\)” for a fixed general \(E\). |
