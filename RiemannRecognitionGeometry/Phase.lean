...
def phaseChangeAngle (I : WhitneyInterval) : Real.Angle :=
...
-/
def phaseChange (I : WhitneyInterval) : ℝ :=
  ‖phaseChangeAngle I‖

/-- Any angle has norm ≤ π. In particular, `phaseChange I ≤ π`. -/
lemma phaseChange_le_pi (I : WhitneyInterval) : phaseChange I ≤ Real.pi := by
  -- General bound: for `AddCircle p`, ‖x‖ ≤ |p|/2.
  have hp : (2 * Real.pi : ℝ) ≠ 0 := by
    nlinarith [Real.pi_pos]
  have h := (AddCircle.norm_le_half_period (p := (2 * Real.pi)) (x := phaseChangeAngle I) hp)
  ...
  -- `phaseChange I = ‖phaseChangeAngle I‖`.
  simpa [phaseChange, hRHS] using h
