...
-- NOTE: this file treats the genuinely hard Calderón–Zygmund / good-λ steps as explicit axioms
-- (see the `axiom` declarations below) so downstream RG code can build while we port proofs.
