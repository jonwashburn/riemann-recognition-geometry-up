...
On the Lean side, the remaining “explicit formula cancellation” work has now been reduced to a single
named full-line integral identity (a statement about the right-edge vertical integral, plus a routine
improper-integral limit lemma). This is recorded in
\texttt{ExplicitFormula/ExplicitFormulaCancellationSkeleton.lean} as
\texttt{rightEdge\_integral\_identity}.
...
The Track 2 formalization is essentially complete with \textbf{zero axioms and zero sorrys}.
All remaining classical-analysis content is carried as explicit hypotheses (so the pipeline compiles and remains modular), and can be proved later.
...
\hline
Axioms (identity analysis input) & 0 & (none; identity inputs are explicit hypotheses/fields, not global axioms) \\
\hline
Axioms (Mathlib gap) & 0 & (none; remaining gaps are explicit hypotheses/fields) \\
\hline
