...
The formalization has the following axioms (no sorrys):

\textbf{1. Classical analysis axiom} (in \texttt{Caratheodory.lean}):
\begin{enumerate}
  \item \texttt{herglotz\_representation} --- the Herglotz representation theorem (1911).
...
\textbf{3. Contour-to-boundary axioms} (in \texttt{ContourToBoundary.lean}):
These axioms decompose the splice completion into smaller, provable pieces (and include a proved complex-linear extension).  (The boundary phase function \(\theta\) is now bundled as data in \texttt{PSCComponents}, not an axiom.)
\begin{itemize}
  \item \texttt{explicit\_formula\_cancellation}: arithmetic terms cancel
  \item \texttt{psc\_phase\_velocity\_identity}: $-w' = \pi\,\mu_{\mathrm{spec}}$
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Contour-to-boundary connection:} axioms for the log-derivative cancellation and boundary limit are in
  \texttt{RiemannRecognitionGeometry/ExplicitFormula/ContourToBoundary.lean}. These include:
  \begin{itemize}
    \item \texttt{psc\_ratio\_unimodular\_boundary}: $|\mathcal J(1/2+it)| = 1$ a.e.
    \item \texttt{log\_deriv\_decomposition}: $\xi'/\xi = (\det_2)'/\det_2 - \mathcal O'/\mathcal O - \mathcal J'/\mathcal J$
    \item \texttt{boundary\_log\_deriv\_imaginary}: $\mathcal J'/\mathcal J = iw'$ on the boundary
    \item \texttt{explicit\_formula\_cancellation}: arithmetic terms cancel via Lagarias Thm 3.1
    \item \texttt{psc\_phase\_velocity\_identity}: $-w' = \pi\,\mu_{\mathrm{spec}}$
  \end{itemize}
...
The Track 2 formalization is essentially complete with \textbf{three axioms and zero sorrys}
(\texttt{herglotz\_representation} plus the two contour-to-boundary axioms:
\texttt{explicit\_formula\_cancellation}, \texttt{psc\_phase\_velocity\_identity}).

\textbf{The single remaining identity lemma (splice completion):}
\begin{itemize}
  \item \texttt{IntegralAssumptions.identity\_integral} in \texttt{PSCSplice.lean}:
    \[
      W^{(1)}(\mathrm{pair}(f,g)) = \int_{\mathbb R} \overline{F_f(t)}\,F_g(t)\,d\mu_{\mathrm{spec}}(t).
    \]
  \item This is the identity claim connecting the Route~3 contour definition of \(W^{(1)}\) to the
    PSC phase--velocity measure \(\mu_{\mathrm{spec}}\).
  \item Once proved, \texttt{RHμ\_spec\_integral} fires (given the constructed \texttt{IntegralAssumptions}) and yields \texttt{RiemannHypothesis}.
  \item Proof sketch: see \texttt{ROUTE3\_IDENTITY\_PART.md} for the contour bookkeeping (steps SC1--SC3).
\end{itemize}

\textbf{Standard-theorem axiom (Herglotz):}
\begin{itemize}
  \item \texttt{herglotz\_representation} in \texttt{Caratheodory.lean}:
    the classical Herglotz representation theorem (1911).
  \item Formalizing this would require Poisson kernel theory, Riesz representation for positive functionals,
    and Fatou's theorem---substantial but routine analysis not currently in Mathlib.
\end{itemize}
...
\hline
Axioms (bridge) & 0 & (none; splice completion is a lemma to be proved, not a global axiom) \\
\hline
Axioms (identity analysis input) & 2 & \texttt{explicit\_formula\_cancellation}, \texttt{psc\_phase\_velocity\_identity} \\
\hline
Axioms (Mathlib gap) & 1 & \texttt{herglotz\_representation} \\
\hline
