...
\subsubsection*{Scope}
We prove, unconditional: (i) a uniform lattice mass gap on the mean-zero sector via OS positivity and a parity-odd two-layer deficit. For the continuum passage we use AF--free NRC inputs (U2: graph-defect $O(a)$ and low-energy projector control) together with proved UEI/equicontinuity and OS1 isotropy on fixed regions (Thms.~\ref{thm:U1-lsi-uei}, \ref{thm:os1-unconditional}; Lem.~\ref{lem:U1-tree-bounds}; Cor.~\ref{cor:U1-uei}; Lem.~\ref{lem:isotropy-restore}). By these, operator-norm NRC and gap persistence yield a strictly positive continuum mass gap with the same slab constant $\gamma_*$. An optional Mosco route is recorded for cross-checks.
...
\begin{itemize}[leftmargin=2em, itemsep=8pt, parsep=4pt]
  ...
  \item \textbf{Interface convex split (constants).} On fixed slabs, there exist $t_0>0$ and $\theta_*>0$ (uniform in $L$; independent of $\beta$) such that $K_{\rm int}^{(a)}\ge \theta_*\, P_{t_0}$. Consequently, $\|K_{\rm int}^{(a)}\|_{L^2_0}\le 1-\theta_*(1-e^{-\lambda_1(G) t_0})$ and $q_*:=\|e^{-aH}\|_{\rm odd}\le 1-\theta_*(1-e^{-\lambda_1(G) t_0})$.
...
\midrule
$t_0$ & Product heat\,–\,kernel time in the interface convex split; Lem.~\ref{lem:beta-L-independent-minorization}, Cor.~\ref{cor:hk-convex-split-explicit}. \\
$\theta_*$ & Heat\,–\,kernel Doeblin weight in the interface convex split; Lem.~\ref{lem:beta-L-independent-minorization}, Cor.~\ref{cor:hk-convex-split-explicit}. \\
...
In the setting of Theorem~\ref{thm:uei-fixed-region}, fix any $a\in(0,a_0]$ with $\beta=\beta(a)\ge \beta_{\min}(R,N)>0$. Let
\[
...
Assume moreover that the coarse--cell factorization and near--identity refresh mechanism of Lemmas~\ref{lem:ucis-A-cell-decomp} and~\ref{lem:single-link-refresh}--\ref{lem:g-one-link-refresh} hold on the slab (so that, on each cell tick, there is a scale--adapted small--ball component of radius $\asymp \beta^{-1/2}$ with weight bounded below uniformly for all sufficiently small $a$).
...
Let $G$ be a compact connected Lie group with bi--invariant metric. There exist constants $c_0,C_0,t_0>0$ depending only on $G$ such that the following holds.
...
\end{lemma}
\begin{proof}
\emph{Target (to be proved).} This is a quantitative local CLT / mixing statement for small--step random walks on compact Lie groups: after $n\asymp\beta$ steps of size $\asymp \beta^{-1/2}$ the walk dominates the heat kernel at a fixed time $t_0\asymp 1$ with constants bounded away from $0$ uniformly in $\beta$.

\smallskip
One approach is to combine (i) comparison of $\nu_\beta$ to an isotropic small--step increment with second moment $\asymp 1/\beta$, (ii) a spectral comparison on each irreducible representation (Fourier analysis on $G$) yielding $\|\widehat{\nu_\beta}(\pi)\|\le e^{-c\,\lambda_\pi/\beta}$ for nontrivial $\pi$, and (iii) inversion/heat kernel bounds to upgrade the spectral decay after $n\ge C_0\beta$ steps to a pointwise lower bound by $p_{t_0}$ (choose $t_0\simeq c\,n/\beta$ and then freeze $t_0$ by taking $n=C_0\beta$). A second approach is via coupling to Brownian motion using Stroock--Varadhan support and a parametrix on $G$, producing a uniform two--sided bound once $n/\beta$ exceeds a fixed threshold.
\end{proof}

\begin{proof}[Proof sketch of Theorem~\ref{thm:ucis-sw}]
Replace UCIS--C by Lemma~\ref{lem:scaled-ball-to-hk}. On each coarse cell, Lemmas~\ref{lem:single-link-refresh}--\ref{lem:g-one-link-refresh} provide a scale--adapted ball component of radius $\asymp \beta^{-1/2}$ with uniform weight; by disjoint support (Lemma~\ref{lem:ucis-A-cell-decomp}) and plaquette factorization the coarse increment law tensorizes across cells as in Lemma~\ref{lem:ucis-D-product-ball}. Under the scaling window \eqref{eq:ucis-sw-window}, the physical slab contains $n\ge C_0\beta(a)$ effective ticks per refreshed cell; apply Lemma~\ref{lem:scaled-ball-to-hk} on each factor and tensorize to obtain a product heat--kernel component $P_{t_0}$ with a uniform weight $\theta_*$, yielding the claimed minorization.
\end{proof}
...
\emph{Step 5 (Bounding $\mathbb{E}S_R$ and conclusion).} In the intended weak-coupling scaling regime, $S_R$ behaves as a Riemann sum for the local continuum density, and one expects $\sup_{a\in(0,a_0]}\mathbb{E}_{\mu_R}S_R<\infty$ on fixed $R$. Denote this bound by $M_R(R,N,\beta_{\min})$. Therefore
\[
...
OS2 on the lattice (Thm.~\ref{thm:os}) yields a positive transfer $T=e^{-aH}$. On a fixed slab, the interface engine (staple window: Thm.~\ref{thm:staple-window}; $\mathrm{SU}(N)$ refresh: Lem.~\ref{lem:SU(N)-refresh}; small-ball $\Rightarrow$ HK: Lem.~\ref{lem:ball-to-hk}/Cor.~\ref{cor:product-ball-to-hk}; sandwich: Prop.~\ref{prop:sandwich}) yields a slab-uniform heat--kernel minorization on the coarse interface,
\(
  K_{\rm int}^{(a)}\ge \theta_* P_{t_0}
\)
(Lemma~\ref{lem:beta-L-independent-minorization}; Cor.~\ref{cor:hk-convex-split-explicit}). Hence $\|K_{\rm int}^{(a)}\|_{L^2_0}\le 1-\theta_*(1-e^{-\lambda_1(G) t_0})$ and, by the compression $T=J^* K J$, $\|e^{-aH}\|_{\rm odd}\le 1-\theta_*(1-e^{-\lambda_1(G) t_0})$ (Thm.~\ref{thm:uniform-odd-contraction}).
