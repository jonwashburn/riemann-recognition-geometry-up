...
In the setting of Theorem~\ref{thm:uei-fixed-region}, let $M_R$ be as in Assumption~\ref{assump:uei-mean} and fix any $a\in(0,a_0]$ with $\beta=\beta(a)\ge \beta_{\min}(R,N)>0$. Let
\[
...
Lemma~\ref{lem:U1-B-smallfield} is the core RG-grade payload: it is designed to force $S_R$ to be $O(1)$ on fixed regions and to supply the small-field regime needed for (i) the mean bound (U1-C), (ii) the Lipschitz bounds used in Herbst (U1-D), and (iii) a usable LSI/concentration inequality (U1-E) via standard compact-manifold tools once the measure is sufficiently concentrated near $\mathbf 1$.
\end{remark}
