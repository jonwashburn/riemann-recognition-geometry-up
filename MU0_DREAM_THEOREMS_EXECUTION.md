...
- **Lemma A\*.1 (integral modular-symbol measure exists)**  
  ...
  - **Concrete starting references (accessible in-repo)**:
    - Vatsal 1999 (author PDF `tmp/ubc/vatsal1999_canonical_periods.pdf`): §1.9–§1.10 (interpolation formulae and the Weierstrass argument upgrading special-value congruences to \(p\)-adic \(L\)-function congruences).
    - Kim–Lee–Ponsinet (`tmp/arxiv/1909.01764/kim-lee-ponsinet-19.tex`): records the “map-level gap” (localization + Coleman map may not preserve mod-\(p\) nonvanishing), so for Dream A we should stay on the **analytic \(L_p\)** side rather than zeta elements unless we can prove primitivity is preserved.
...
- **Lemma A\*.2 (divisibility in \(\Lambda\) \(\Leftrightarrow\) mod-\(p\) vanishing of the measure)**  
  ...
  - **Concrete starting reference (accessible in-repo)**:
    - Vatsal 1999, Theorem (1.10): uses Weierstrass preparation to upgrade “values divisible by \(\pi^r\) at infinitely many cyclotomic points” into “power series divisible by \(\pi^r\)”. This is the exact analytic template for Lemma A\*.2 (with \(r=1\)).
...
- **Lemma A\*.3 (mod-\(p\) vanishing \(\Rightarrow\) congruence prime)**  
  ...
  - **Concrete “where the hypotheses live” pointers (accessible in-repo)**:
    - Vatsal 1999, §1.2–§1.3: isolates the cohomology freeness/Gorenstein-style hypotheses (“Condition 1/2” in his notation) needed to identify Hecke eigenspaces integrally; these are the exact points where Lemma A\*.3 would need modularity-lifting inputs (multiplicity one, Ihara lemma, freeness of Hecke modules).
    - Maksoud (`tmp/arxiv/2312.16706/adjoint_arxiv_v1.tex`), Proposition `prop:congruence_module_and_p_adic_L_functions`: a proven instance of the “congruence module is generated by a \(p\)-adic \(L\)” bridge, but in an **adjoint/Hida-family** setting. Structurally, Lemma A\*.3 should look like a cyclotomic analogue of this proposition.

---

## Route 2 (internal proof) — Formalize Lemma A\*.3 and its sub-blockers

The goal here is **not** to write a full proof yet, but to pin down the *exact* statement we need in a form that:

- can be attacked internally, and
- makes every missing ingredient explicit (so we can later cite or prove each one cleanly).

### Lemma A\*.3 (formal target statement; cohomology/Hecke form)

Fix a modular elliptic curve \(E/\mathbb Q\) of conductor \(N\) with associated weight‑2 newform \(f\).
Let \(p\nmid N\) be a good ordinary prime.

Let \(\Gamma=\Gamma_1(Np)\) and let \(\mathbb T=\mathbb T_{\mathbb Z_p}\) denote the \(\mathbb Z_p\)-Hecke algebra acting on the parabolic cohomology
\[
  H^1_{\mathrm{par}}(\Gamma,\mathbb Z_p)\,,
\]
and let \(\mathfrak m\subset \mathbb T\) be the maximal ideal corresponding to \(f\bmod p\) (equivalently the residual representation \(\bar\rho_{f,p}\)).

Let
\[
  \mathrm{MS}_p:\ H^1_{\mathrm{par}}(\Gamma,\mathbb Z_p)_{\mathfrak m}\ \longrightarrow\ \Lambda=\mathbb Z_p\llbracket T\rrbracket
\]
be the (ordinary, cyclotomic) **modular‑symbol/measure map** whose output is the cyclotomic \(p\)-adic \(L\)-function \(L_p(E,T)\) up to \(\Lambda^\times\) (Mazur–Tate–Teitelbaum / Stevens normalization, with canonical-period scaling absorbed into \(\Lambda^\times\)).

Then the desired implication is:

> **Lemma A\*.3 (target).**  
> If \(\mathrm{MS}_p(\delta_f)\in p\Lambda\) for the cohomology eigenclass \(\delta_f\) attached to \(f\) (i.e. \(L_p(E,T)\in p\Lambda\)), then \(p\mid \mathfrak c(f)\) (equivalently \(p\mid C(E)\)).

Equivalently (contrapositive form, closer to what one actually proves):

> If \(p\nmid \mathfrak c(f)\), then \(\mathrm{MS}_p(\delta_f)\notin p\Lambda\) (hence \(\mu_p(E)=0\)).

### Why this is the right “mod-\(p\) cohomology/Hecke” core

- “\(L_p(E,T)\in p\Lambda\)” means the measure is \(0\bmod p\), i.e. all cyclotomic specializations are \(0\bmod p\).
- A modular‑symbol description expresses those specializations as pairings of \(\delta_f\) with a family of explicit homology classes (Manin symbols / \(\Lambda(\chi)\)-classes).
- If those homology classes span the localized mod‑\(p\) homology, then all pairings being \(0\bmod p\) forces \(\delta_f\equiv 0\bmod p\).
- Under a freeness/Gorenstein hypothesis, \(\delta_f\equiv 0\bmod p\) is equivalent to “the \(f\)-eigenspace is not isolated integrally”, i.e. \(p\mid\mathfrak c(f)\).

### Sub-blockers (explicit requirements to make Lemma A\*.3 go through)

We now list the *minimal* sub-lemmas that together would prove Lemma A\*.3. Each should be treated as a separate import/proof task.

- **A\*.3a — Integral modular-symbol realization (definition-level)**  
  Define \(\delta_f\in H^1_{\mathrm{par}}(\Gamma,\mathbb Z_p)_{\mathfrak m}\) canonically up to \(\mathbb Z_p^\times\), and define \(\mathrm{MS}_p\) so that \(\mathrm{MS}_p(\delta_f)\) agrees with \(L_p(E,T)\) up to \(\Lambda^\times\).
  - **Depends on**: integral Eichler–Shimura + normalization dictionary for periods (ties into Vatsal’s “canonical period up to \(p\)-unit” framing).

- **A\*.3b — Spanning/duality statement (the “Stevens span” input)**  
  Let \(V:=H^1_{\mathrm{par}}(\Gamma,\mathbb F_p)_{\mathfrak m}\). Show that the set of homology classes \(\{\Lambda(\chi)\}\) (or Manin symbols) coming from characters of \(p\)-power conductor span \(V^\vee\) (or equivalently their pairings detect nonzero cohomology classes).
  - **This is where** mod‑\(p\) multiplicity one and Ihara‑lemma style inputs typically enter.

- **A\*.3c — Detection lemma (values control the class)**  
  Prove: if \(\mathrm{MS}_p(\delta)\equiv 0\bmod p\) in \((\mathbb Z/p)\llbracket T\rrbracket\), then \(\delta\equiv 0\bmod p\) in \(H^1_{\mathrm{par}}(\Gamma,\mathbb F_p)_{\mathfrak m}\).  
  (This is the formalization of “all specializations vanish \(\Rightarrow\) class vanishes”, using A\*.3b.)

- **A\*.3d — Freeness/Gorenstein control (isolation of the eigenspace)**  
  Show: if \(p\nmid \mathfrak c(f)\), then \(H^1_{\mathrm{par}}(\Gamma,\mathbb Z_p)_{\mathfrak m}\) is \(\mathbb Z_p\)-torsion-free and the relevant localized Hecke algebra \(\mathbb T_{\mathfrak m}\) is Gorenstein (or at least the cohomology eigenspace is free of rank \(1\) over \(\mathbb Z_p\) on the \(f\)-branch), so that \(\delta_f\not\equiv 0\bmod p\).
  - **This is the explicit “congruence ideal controls integrality” hinge**: it is where we pay for the definition of \(C(E)\).

- **A\*.3e — Congruence ideal dictionary (bridge to \(C(E)\))**  
  Prove the equivalence between “failure of freeness/Gorenstein at \(\mathfrak m\)” and “\(p\mid \mathfrak c(f)\)” for the chosen definition of \(\mathfrak c(f)\subset \mathbb Z\).
  - **Expected inputs**: Wiles-style congruence number formalism; Hecke-module perfect pairings; identification of congruence modules with annihilators/Fitting ideals.

### Net effect

If we can prove A\*.3a–A\*.3e, then Lemma A\*.3 (and hence Dream A) becomes a theorem with an explicit finite exceptional set \(\{p: p\mid C(E)\}\cup S_{\mathrm{bad}}\).
...
- **Search log (what we already checked)**:
  ...
  - Vatsal (Duke Math. J. **98** (1999), *Canonical periods and congruence formulae*): **FOUND** as an author-hosted PDF: `https://www.math.ubc.ca/~vatsal/research/bah.PDF` (local copy: `tmp/ubc/vatsal1999_canonical_periods.pdf`). It gives:
    - Theorem **(0.2)** + Proposition **(1.7)**: canonical periods and congruence of algebraic special values under cuspform congruences \(f\equiv g\ (\bmod\,p^r)\).
    - Theorem **(1.10)**: congruence of \(p\)-adic \(L\)-functions under such congruences (via Weierstrass preparation).
    - Theorem **(2.10)**: Eisenstein congruence case with a \(p\)-adic unit period \(\Omega_E\) and Washington-type nondivisibility input on the Eisenstein side.
    - **What it does *not* supply**: an explicit cyclotomic primitivity bridge of the form “\(p\mid L_p(E,T)\Rightarrow p\mid C(E)\)” for a fixed general non‑CM \(E/\mathbb Q\). In particular, it uses cohomological freeness/Gorenstein hypotheses to define canonical periods and prove congruence statements, and it treats nondivisibility in special Eisenstein-congruence settings; it does not close Dream A in general.
